...
def glob():
    ...
    start_url="https://imperial-online.kz/katalog/suxie-smesi-46567/shtukaturnyie-suxie-smesi-46579.html"
...
def details(url):
    try:       
        ...
        # Поиск информации
        try:
            # Категория /html/body/div[2]/div/div[1]/ul/li[4]/a
            kategoriya = driver.find_element(By.XPATH, "/html/body/div[2]/div/div[1]/ul/li[4]/a")
            #row.append(kategoriya.text)
            row.append("Штукатурные сухие смеси")            
            ...
            print(exception)
        try:
            # Название /html/body/div[2]/div/div[2]/div/div[2]/form/h1
            #nazvanie = driver.find_element(By.XPATH, "/html/body/div[2]/div/div[2]/div/div[2]/form/h1")
            #row.append(nazvanie.text)
            nazvanie = driver.find_element(By.TAG_NAME, "h1")  
            row.append(nazvanie.text[0:nazvanie.text.find("\n")])
            print(nazvanie.text[0:nazvanie.text.find("\n")])            
        except Exception as exception: 
            row.append("nazvanie")
            #nazvanie = "nazvanie"
            print(exception)        
        try:
            #Описание /html/body/div[2]/div/div[2]/div/div[2]/form/div[6]/div/div[2]
            #description = driver.find_element(By.XPATH, "/html/body/div[2]/div/div[2]/div/div[2]/form/div[6]/div/div[2]")
            description = driver.find_element(By.CLASS_NAME, "body-content")
            if description != None:
        ...
        try:
            #Цена /html/body/div[2]/div/div[2]/div/div[2]/form/div[2]/div/div[2]
            #cena = driver.find_element(By.XPATH, "/html/body/div[2]/div/div[2]/div/div[2]/form/div[2]/div/div[2]")
            cena = driver.find_element(By.CLASS_NAME, "product-page-price")
            if cena != None:
            ...
            print(exception)
        try:
            # Артикул /html/body/div[2]/div/div[2]/div/div[2]/form/h1
            #artikul = driver.find_element(By.XPATH, "/html/body/div[2]/div/div[2]/div/div[2]/form/h1")
            artikul = driver.find_element(By.TAG_NAME, "h1")  
            row.append(artikul.text[nazvanie.text.find("\n")+1:])
            print(artikul.text[nazvanie.text.find("\n")+1:])      
        except Exception as exception: 
            row.append("artikul")
            #artikul = "artikul"
            print(exception)        
        try:
            # Фотография //*[@id="msGallery"]/div[1]/a/img
            #img = driver.find_element(By.XPATH, "//*[@id="msGallery"]/div[1]/a/img")
            # Поиск ссылок
            links = driver.find_elements(By.TAG_NAME, "a")
            img_links = []
            for link in links:
                if str(link.get_attribute('href'))[0:50]=="https://imperial-online.kz/assets/images/products/":
                    if img_links.count(str(link.get_attribute('href')))==0:
                        img_links.append(str(link.get_attribute('href')))
...
def parsing(url):
    try:
        ...
        # Поиск ссылок
        links = driver.find_elements(By.TAG_NAME, "a")
        # Набор ссылок на карточки, пример ссылки на карточки https://imperial-online.kz/katalog/suxie-smesi-46567/shtukaturnyie-suxie-smesi-46579/shtukaturka-bergauf-(koroed)-dekor-25kg-2359146.html
        my_links = []
        for link in links:
            if str(link.get_attribute('href'))[0:85]=="https://imperial-online.kz/katalog/suxie-smesi-46567/shtukaturnyie-suxie-smesi-46579/":
        ...
        for i in range(len(my_links)):     
            try:
                ...
                if (len(glob.data) % 1000 == 0):
                    ...
                    excel.export_to_excel(HEADER, glob.data, "imperial.kz." + str(current_date_time) + ".xls")
...
def run():    
    try:
        ...
        # Вызов: заголовок таблицы, данные, название файла
        current_date_time = datetime.now().strftime('%Y%m%d%H%M%S')
        excel.export_to_excel(HEADER, glob.data, "imperial.kz.range(" +str(glob.start_page) + "-" + str(glob.finish_page) + ")_" +str(current_date_time) + ".xls")
        # Экспорт в БД
...
# Создается новый класс ImperialWindow котрый наследуется от класса QMainWindow. 
class ImperialWindow(QMainWindow):
    def __init__(self):
        try:
            super().__init__()
            ...
            # Заголовок окна
            self.setWindowTitle("imperial.kz")
            ...
            self.lineEdit_start_url.setText("https://imperial-online.kz/katalog/suxie-smesi-46567/shtukaturnyie-suxie-smesi-46579.html")
            ...
            self.spinBox_finish_page.setValue(2)
