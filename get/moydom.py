...
def glob():
    ...
    start_url="https://moydom.kz/catalog/suhie-smesi-17/"
...
def details(url):
    try:       
        ...
        # Поиск информации
        try:
            # Категория //*[@id="layout-default"]/div/main/section/section[1]/div/ul/li[6]/a/span
            #kategoriya = driver.find_element(By.XPATH, "//*[@id="layout-default"]/div/main/section/section[1]/div/ul/li[5]/a/span")
            kategoriyas = driver.find_elements(By.CLASS_NAME, "breadcrumbs--mobile")
            #for kategoriya in kategoriyas:
            #    print(kategoriya.text)  
            row.append(kategoriyas[0].text)
            print(kategoriyas[0].text)
        except Exception as exception:
            row.append("kategoriya")
            #kategoriya = "kategoriya"
            print(exception)
        try:
            # Название //*[@id="layout-default"]/div/main/section/section[2]/div/section/div[1]/div[1]/h1
            #nazvanie = driver.find_element(By.XPATH, "//*[@id="layout-default"]/div/main/section/section[2]/div/section/div[1]/div[1]/h1")
            ...
            print(exception)
        try:
            #Описание //*[@id="layout-default"]/div/main/section/section[3]/div[3]/div/div/div/div/div[1]/div[1]
            #description = driver.find_element(By.XPATH, "//*[@id="layout-default"]/div/main/section/section[3]/div[3]/div/div/div/div/div[1]/div[1]")
            #description = driver.find_element(By.CLASS_NAME, "product-specs")
            #if description != None:
            #    row.append(description.get_attribute("innerHTML").strip())
            ...
            # Ключи
            my_key = []
            div_elements = driver.find_elements(By.CSS_SELECTOR, "div.product-specs__key")
            for div_element in div_elements:
                if div_element.get_attribute("innerHTML") != None:
                    if div_element.get_attribute("innerHTML").strip() != "":
                        my_key.append(div_element.get_attribute("innerHTML").strip())
                        #print(div_element.get_attribute("innerHTML").strip())
            # Значение
            my_value = []
            div_elements = driver.find_elements(By.CSS_SELECTOR, "div.product-specs__value")
            for div_element in div_elements:
                if div_element.get_attribute("innerHTML") != None:
                    if div_element.get_attribute("innerHTML").strip() != "":
                        my_value.append(div_element.get_attribute("innerHTML").strip())
                        #print(div_element.get_attribute("innerHTML").strip())
            # Пара ключ значение
            description = ""
            for i in range (len(my_key)):
                description = description + my_key[i] + ": " + my_value[i].replace("<span itemprop=\"weight\" class=\"app-weight\" data-v-9704686e=\"\">", "").replace("</span>", "")  + "\n" 
                #print(my_key[i] + ": " + my_value[i])
            print(description)
            row.append(description)            
        except Exception as exception: 
            row.append("description")
        ...
        try:
            #Цена //*[@id="layout-default"]/div/main/section/section[2]/div/section/div[2]/div[2]/div/div/div/div[1]/div/div/div[1]/span[1]
            #cena = driver.find_element(By.XPATH, "//*[@id="//*[@id="layout-default"]/div/main/section/section[2]/div/section/div[2]/div[2]/div/div/div/div[1]/div/div/div[1]/span[1]")
            cena = driver.find_element(By.CLASS_NAME, "product-price__block")
            if cena != None:
            ...
            print(exception)
        try:
            # Артикул //*[@id="layout-default"]/div/main/section/section[2]/div/section/div[1]/div[2]
            #artikul = driver.find_element(By.XPATH, "//*[@id="layout-default"]/div/main/section/section[2]/div/section/div[1]/div[2]")
            artikul = driver.find_element(By.CLASS_NAME, "product-header__article")
            if artikul != None:
            ...
            print(exception)
        try:
            # Фотография //*[@id="layout-default"]/div/main/section/section[2]/div/section/div[2]/div[1]/div/div[2]/div/div/img
            #img = driver.find_element(By.XPATH, "//*[@id="layout-default"]/div/main/section/section[2]/div/section/div[2]/div[1]/div/div[2]/div/div/img")
            img = driver.find_element(By.CLASS_NAME, "product-images-main-image__img")
...
def parsing(url):
    try:
        ...
        # Набор ссылок на карточки, пример ссылки на карточки https://moydom.kz/model/1738/
        my_links = []
        for link in links:
            if str(link.get_attribute('class'))=="snippet-wrapper-link product-snippet__wrapper product-snippet__wrapper--link":
        ...
        for i in range(len(my_links)):     
            try:
                ...
                if (len(glob.data) % 1000 == 0):
                    ...
                    excel.export_to_excel(HEADER, glob.data, "moydom.kz." + str(current_date_time) + ".xls")
...
def run():    
    try:
        ...
        # Вызов: заголовок таблицы, данные, название файла
        current_date_time = datetime.now().strftime('%Y%m%d%H%M%S')
        excel.export_to_excel(HEADER, glob.data, "moydom.kz.range(" +str(glob.start_page) + "-" + str(glob.finish_page) + ")_" +str(current_date_time) + ".xls")
        # Экспорт в БД
...
# Создается новый класс MoydomWindow котрый наследуется от класса QMainWindow. 
class MoydomWindow(QMainWindow):
    def __init__(self):
        try:
            super().__init__()
            ...
            self.lineEdit_start_url.setText("https://moydom.kz/catalog/suhie-smesi-17")
            ...
            self.spinBox_finish_page.setValue(10)
